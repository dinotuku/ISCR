...
def run_reconstruct_query(cmvn_model_file, query_big5_file, query_utf8_file):
    ...
    os.system(
        "iconv -f big5 -t utf-8 {} > {}".format(query_big5_file, query_utf8_file))
...
def run_transcript2docmodel(query_utf8_file, transcript_dir, lex_file,
    ...
    if not os.path.exists(lex_file):
        ...
        for docname in tqdm(sorted(os.listdir(transcript_dir))):
            ...
            with open(doc_path, 'r') as f:
                ...
                # Remove whitespace & endline
                split_uni_text = ''.join(uni_text.split())
                # Split with jieba
                cut_uni_text = list(jieba.cut(split_uni_text))
    ...
    if not os.path.exists(docmodels_cache):
        print("Reading docmodels from {} and saving cache to {}".format(
            transcript_dir, docmodels_cache))
        docmodels = {}
        doclength_dict = {}
    ...
    if not os.path.exists(save_docmodel_dir):
        ...
        print("Document models have already been written to {}".format(
            save_docmodel_dir))
...
def run_create_requests(docmodel_dir, inv_index_file, doclength_file, request_dir):
    if not os.path.exists(request_dir):
        ...
        for doc_fname in tqdm(glob(os.path.join(docmodel_dir, '*'))):
            ...
            sorted_model = sorted(docmodel.iteritems(),
...
def run_create_keyterms(inv_index_file, keyterm_dir):
    if not os.path.exists(keyterm_dir):
        ...
        for term1 in tqdm(inv_index.keys()):
            ...
            listToPrint = sorted(jaccard.iteritems(),
                                 key=operator.itemgetter(1), reverse=True)
...
def run_create_lda(mallet_binary, docmodel_dir, lda_dir, lex_file):
    ...
    topic_words_weight_file = os.path.join(
        train_dir, 'topic_words_weight_file')
    ...
    if os.path.exists(topic_words_weight_file):
        print("Topic words weight file already exists {}. Skipping...".format(
...
def run_create_topic_rankings(mallet_binary, query_utf8_jieba_file, lda_dir, topic_ranking_dir):
    ...
    document_mallet_file = os.path.join(
        lda_dir, 'mallet_train', 'documents.mallet')
    ...
    if os.path.exists(query_mallet_file):
        ...
        os.system(import_file_cmd.format(
            mallet_binary, query_utf8_jieba_file, query_mallet_file, document_mallet_file))

    # Infer Model
    print("Inferencing quries")
    inferencer_model = os.path.join(
        lda_dir, 'mallet_train', 'inferencer.model')
    query_doc_topics = os.path.join(
        lda_dir, 'mallet_train', 'query_doc_topics.txt')
    ...
    if os.path.exists(query_doc_topics):
        ...
        os.system(inference_cmd.format(mallet_binary,
                                       query_mallet_file, inferencer_model, query_doc_topics))
...
def cut_queries(query_utf8_file, query_utf8_jieba_file):
    if os.path.exists(query_utf8_jieba_file):
        print("Jieba query file has already been cut {}".format(
            query_utf8_jieba_file))
        return
...
def cut_transcript(transcript_dir, jieba_dir):
    if not os.path.exists(jieba_dir):
        ...
        print("Run cutting transcript from {} to {}".format(
            transcript_dir, jieba_dir))
        ...
        print("Transcript {} has already been cut to {}".format(
            transcript_dir, jieba_dir))
...
#    transcript_name   = 'transcript'
    ...
    run_reconstruct_query(cmvn_model_query_file,
                          query_big5hex_file, query_utf8_file)
    ...
    run_create_query_pickle(lex_file, query_utf8_file,
                            answer_file, query_pickle)
    ...
    run_create_requests(save_docmodel_dir, index_file,
                        doclength_file, request_dir)
    ...
    run_create_topic_rankings(
        mallet_binary, query_utf8_jieba_file, lda_dir, topic_ranking_dir)
