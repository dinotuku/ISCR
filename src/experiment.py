...
class Experiment(object):
    ...
    def run_episode(self,q,ans,ans_index,max_steps,test_flag=False):
        ...
        while True:
            if action == 4:
                reward, state, ret = self.env.step(action) # ENVIRONMENT STEP
            else:
                reward, feature, request, feedback = self.env.step(action)
            ...
            if test_flag: # and action != 4:
                ...
                if action == 4:
                    logging.debug('final retrieve: %s', str(ret['ret']))
                else:
                    # logging.debug('retrieve: %s', str(request['ret']))
                    logging.debug('feedback: %s', str(feedback))
